...
3. **`INDEX.md` を更新**
4. **`README.md` を更新（必要なとき）** — 訪問者向けの入口で、すべての追加で更新が必要なわけではない。下記の判定に従う

### README.md の更新トリガー

`README.md` は訪問者向けの厳選された入口。`INDEX.md` のように全件を載せるのではない。次のいずれかに当てはまるときだけ更新する:

- [ ] **新しいベンダー/ツールを取り込んだ** → 「ベンダー/ツール別の入口」テーブルの行を「枠のみ」から「取り込み済み」に切替、または新規行追加
- [ ] **新しい frameworks/ ノートを作った** → 上記テーブルの `frameworks` 列を「—」から実リンクに更新
- [ ] **新しい業務領域 (domain) を稼働させた** → 「業務領域別の入口」テーブルの状態列を更新
- [ ] **訪問者が「判断したい」典型ケースに関する frameworks/topics を追加した** → 「2. 判断する」テーブルに行を追加。ただしリポ全体で 10 行を超えないよう、優先度の低いものは INDEX.md に任せる
- [ ] **訪問者が必ず辿りたい代表的な原典を取り込んだ** → 「3. 原典で確認する」テーブルに 1〜2 行追加（全件は INDEX.md）
- [ ] **新しい playbooks/ を作った** → 「1. まず始める」テーブルに追加
- [ ] **`README.md` L3 のイントロで挙げているカテゴリに該当する新ベンダーを取り込んだ** → 列挙を更新

判定で迷ったら「訪問者がトップページを読んだだけで存在を知るべきか？」で考える。`INDEX.md` は「現在何があるか」、`README.md` は「何から読むべきか」の役割分担。
...
- [ ] **`INDEX.md` を更新したか** — 現在の全件を反映
- [ ] **`README.md` の更新トリガーに当てはまらないか** — 上記「README.md の更新トリガー」を満たしていれば必ず更新。新ベンダー追加・新 frameworks ノート作成・新業務領域の稼働は**ほぼ必ず**当てはまる
